...
for(i in 1:nrow(mydata)) {
    row <- mydata[i,]
    variances[i] = varianceH(row, lambda.est, psi.est, sigma.est, nu.est, b.est, c.est)
}
...
expectations = matrix(, nrow = nrow(mydata), ncol = k)
variances2 = matrix(, nrow = nrow(mydata), ncol = k)
for(i in 1:nrow(mydata)){
    z <- mydata[i,1:p]
    a <- mydata[i,p+1]
    ...
    constant = c(a/pnorm(inner.constant) + (1-a)/(1-pnorm(inner.constant)))
    meanU = constant*cubintegrate(meanF, rep(-Inf,k), rep(Inf,k), fDim=k)$integral
    expectations[i,] = meanU
    
    varF = u2CDF.generate(M.inv.est, d.est, b.est, c.est)
    varU = constant*cubintegrate(varF, rep(-Inf,k), rep(Inf,k), fDim=k)$integral
    variances2[i,] = varU
}
