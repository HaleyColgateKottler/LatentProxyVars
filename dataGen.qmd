...
C1 = 0.3 * t(rep(1,k))
C2 = 0.7 * t(rep(1,k))
save(k, p, communality, var_A, var_Y, n, sigma, lambda, psi, H_mean, H_covar, B, C0, C1, C2, A_intercept, Z_intercept, Y_intercept, file = paste("param_", tag, ".RData", sep = ""))
```
...
for (i in 1:n){
    ...
    Y = A * (C1 %*% H) + (1-A) * (C2 %*% H) + C0 * A + Y_intercept + epsilon_Y
