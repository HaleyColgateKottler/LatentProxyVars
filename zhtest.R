...
p <- 3
savemarker <- 10
dataGenNCE(k, p, 3, 1, 1000, .3, c(.5, .7, .9), 2, matrix(c(1),
  nrow = 1, byrow = TRUE
), .5, c(-.8), c(-.2, .9, .7), tag)
sampleSize <- 1000
...
for (j in 0:99) {
  ...
  for (h.weight.base in 0:10) {
    ...
    for (entry.row in 1:1000) {
      ...
      rawData$Y[entry.row] <- (1 - h.weight) * (C2 %*% t(zhrow[1:p])) +
        h.weight * (C1 %*% t(zhrow[(p + 1):(p + k)])) + C0 * rawData$A[entry.row] + Y_intercept + epsilon_Y
    }
